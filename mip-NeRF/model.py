...
class PositionalEncoding(nn.Module):
    def __init__(self, min_deg, max_deg):
        ...
        self.scales = nn.Parameter(jittor.float32([2 ** i for i in range(min_deg, max_deg)]), requires_grad=False)

    def execute(self, x, y=None):
        shape = list(x.shape[:-1]) + [-1]
        x_enc = (x[..., None, :] * self.scales[:, None]).reshape(shape)
        x_enc = jittor.concat((x_enc, x_enc + 0.5 * numpy.pi), -1)
        if y is not None:
            # IPE
            y_enc = (y[..., None, :] * self.scales[:, None]**2).reshape(shape)
            y_enc = jittor.concat((y_enc, y_enc), -1)
            x_ret = jittor.exp(-0.5 * y_enc) * jittor.sin(x_enc)
            y_ret = jittor.maximum(jittor.zeros_like(y_enc), 0.5 * (1 - jittor.exp(-2 * y_enc) * jittor.cos(2 * x_enc)) - x_ret ** 2)
            return x_ret, y_ret
        else:
            # PE
            x_ret = jittor.sin(x_enc)
            return x_ret
        ...
        for l in range(self.num_levels):
            ...
            if l == 0:  # coarse grain sample
                ...
                t_vals, (mean, var) = resample_along_rays(rays.origins, rays.directions, rays.radii,
                                                          t_vals,
                                                          weights, randomized=self.randomized,
            ...
            new_encodings = jittor.concat((new_encodings, samples_enc), -1)
            ...
            if self.use_viewdirs:
                ...
                viewdirs = self.viewdirs_encoding(rays.viewdirs)
                viewdirs = jittor.concat((viewdirs, rays.viewdirs), -1)
                viewdirs = numpy.tile(viewdirs[:, None, :], (1, self.num_samples, 1))
                viewdirs = viewdirs.reshape((-1, viewdirs.shape[-1]))
                new_encodings = self.rgb_net0(new_encodings)
                new_encodings = jittor.concat((new_encodings, viewdirs), -1)
            ...
            if self.randomized and self.density_noise:
                raw_density += self.density_noise * jittor.rand(raw_density.shape, dtype=raw_density.dtype)

            # volumetric rendering
            rgb = raw_rgb * (1 + 2 * self.rgb_padding) - self.rgb_padding
            density = self.density_activation(raw_density + self.density_bias)
            comp_rgb, distance, acc, weights, alpha = volumetric_rendering(rgb, density, t_vals, rays.directions, self.white_bkgd)
            comp_rgbs.append(comp_rgb)
            distances.append(distance)
            accs.append(acc)
        if self.return_raw:
            raws = jittor.concat((jittor.clone(rgb).detach(), jittor.clone(density).detach()), -1)
            # Predicted RGB values for rays, Disparity map (inverse of depth), Accumulated opacity (alpha) along a ray
            return jittor.stack(comp_rgbs), jittor.stack(distances), jittor.stack(accs), raws
        else:
            # Predicted RGB values for rays, Disparity map (inverse of depth), Accumulated opacity (alpha) along a ray
            return jittor.stack(comp_rgbs), jittor.stack(distances), jittor.stack(accs)
    ...
    def render_image(self, rays, height, width, chunks=8192):
        ...
        with jittor.no_grad():
            for i in range(0, length, chunks):
                ...
                chunk_rays = namedtuple_map(lambda r: r[i:i+chunks], rays)
                print("turns no."+str(i)+" after mapping")
                rgb, distance, acc = self(chunk_rays)
                print("turns no."+str(i)+" after forward")
                rgbs.append(rgb[-1])
                dists.append(distance[-1])
                accs.append(acc[-1])

        rgbs = to8b(jittor.concat(rgbs, dim=0).reshape(height, width, 3).numpy())
        print("correct rgbs, "+str(rgbs))
        dists = jittor.concat(dists, dim=0).reshape(height, width).numpy()
        print("correct dists"+str(dists))
        accs = jittor.concat(accs, dim=0).reshape(height, width).numpy()
        print("correct accs"+str(accs))
