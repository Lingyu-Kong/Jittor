...
class Runner():
    ...
    def train(self):
        max_val_psnr = 0
        last_max_val_psnr = 0
        if os.path.exists(os.path.join(self.save_path, "test/args.json")):
            with open(os.path.join(self.save_path, "test/args.json"), "r") as f:
                args = json.load(f)
                last_max_val_psnr = args["PSNR"]
                max_val_psnr = args["PSNR"]
        for i in tqdm(range(self.tot_train_steps)):
            ...
            if i>0 and i%self.val_freq==0:
                ...
                if i > 20000 and psnr > max_val_psnr:
                    max_val_psnr = psnr
                    self.test()
        if max_val_psnr > last_max_val_psnr:
            with open(os.path.join(self.save_path, "test/args.json"), "w") as f:
                args = {"aabb_scale": self.dataset["train"].aabb_scale,
                        "scale": self.dataset["train"].scale,
                        "offset": self.dataset["train"].offset,
                        "total_step": self.tot_train_steps,
                        "near_distance": self.sampler.near_distance,
                        "PSNR": max_val_psnr.numpy().tolist()
                        }
